...
#if 0
    update_info(AUTH_ROOT, NULL, id, UPDATE_ALL);
    update_info(AUTH_USER, NULL, id, UPDATE_ALL);
    ui_refresh();
#endif
}
                             ...
                             gpointer user_data)
{
    ...
    /* Get certificate ID */
    gtk_tree_model_get(model, &iter, param->id_col, &cert_id, -1);

    window = gtk_widget_get_ancestor(GTK_WIDGET(tree),
                                     GTK_TYPE_WINDOW);

    /* Show certificate details dialog and update info */
#if 0
    certmanui_certificate_details_dialog(window, param->simple, NULL);
#endif
